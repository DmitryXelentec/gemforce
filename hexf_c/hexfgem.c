...
#include <unistd.h>
#include <string.h>
typedef struct Gem_HF gem;		// the strange order is so that managem_utils knows which gem type are we defining as "gem"
#include "hexfgem_utils.h"
...
void worker(int len, int output_options, int size)
{
	...
	gem_init(gems   ,1,1,1);		// grade damage firerate
