...
void worker(int len, int output_options)
{
	...
	if (output_options & mask_upto) {
		...
		int best_index=0;
		for (i=0; i<len; ++i) {
			if (log(gems[i].suppr)/log(i+1) > best_growth) {
				best_index=i;
				best_growth=log(gems[i].suppr)/log(i+1);
			}
		}
