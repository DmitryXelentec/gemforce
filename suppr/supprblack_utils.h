#ifndef _SUPPRBLACK_UTILS_H
#define _SUPPRBLACK_UTILS_H

const int ACC=100;			// accuracy for comparisons
...
struct Gem_SB {
	...
	float suppr;
	float bbound;
	struct Gem_SB* father;
	struct Gem_SB* mother;
};
...
{
	return gem1.suppr*gem1.bbound;     // amp-less
}

char gem_color(gem* p_gem)
{
	if (p_gem->suppr==0 && p_gem->bbound==0) return 'r';
	if (p_gem->suppr==0) return 'b';
	if (p_gem->bbound==0) return 's';
	else return 'h';
}
...
inline double gem_cfr_power(gem gem1, double amp_suppr, double suppr_ratio)
{
	if (gem1.suppr==0) return 0;
	return (gem1.suppr+suppr_ratio*amp_suppr)*gem1.bbound;
}

gem gem_putred(gem* pool, int pool_length, int value, gem* red, gem** gem_array, double amp_suppr, double suppr_ratio)
{
	int isRed;
	int last;
	int curr;
	...
	for (i=0; i<pool_length; ++i) {
		...
		for (last=0; last<value; last++) {
			...
			double new_pow=gem_cfr_power(*gp, amp_suppr, suppr_ratio);
			if (new_pow > best_pow) {
				best_pow=new_pow;
